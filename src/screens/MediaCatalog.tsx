...
export function MediaCatalog({ kind, open }: Props) {
  ...
  return <main className="screen catalog-screen"><header className="section-header"><div><span className="eyebrow">ON DEMAND</span><h1>{title}</h1></div><SearchField focusKey={searchKey} value={search} input={setSearch} placeholder={`Search ${title.toLowerCase()}`} neighbors={{ down: [PREVIOUS_FOCUS, firstItemKey || categoryKey] }} /></header>
    <div className="media-layout"><aside className="category-panel"><h2>Categories</h2>{categories.loading ? <Spinner /> : categories.error ? <ErrorState error={categories.error} retry={categories.retry} /> : <div className="category-list">
      <Focusable focusKey={`${kind}:cat:all`} group={`${kind}-cats`} index={0} orientation="vertical" neighbors={{ right: firstItemKey, up: searchKey }} onSelect={() => setCategory('')} className={!category ? 'active-item' : ''}>All {title.toLowerCase()}</Focusable>
