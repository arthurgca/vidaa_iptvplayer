...
export function LiveTV({ category, setCategory, play }: LiveTVProps) {
  ...
  return <main className="screen catalog-screen">
    <header className="section-header"><div><span className="eyebrow">LIVE TV</span><h1>Channels</h1></div><SearchField focusKey="live:search" value={search} input={setSearch} placeholder="Search channels" neighbors={{ down: [PREVIOUS_FOCUS, searchReturn] }} /></header>
    <div className="live-layout">
      <aside className="category-panel"><h2>Categories</h2>{categories.loading ? <Spinner /> : categories.error ? <ErrorState error={categories.error} retry={categories.retry} /> : <div className="category-list">
        <Focusable focusKey="live:cat:all" group="live-cats" index={0} orientation="vertical" neighbors={{ right: selected ? `live:channel:${selected.id}` : undefined, up: 'live:search' }} onSelect={() => { setCategory(''); setSelected(undefined); }} className={!category ? 'active-item' : ''}>All channels</Focusable>
      ...
      <aside className="epg-panel">{selected ? <>
        ...
        <Focusable focusKey="live:favorite" group="live-info" index={0} orientation="vertical" neighbors={{ left: `live:channel:${selected.id}`, up: 'live:search' }} onSelect={toggleFavorite} className="epg-favorite">{favorite ? '★ Favorite' : '☆ Add Favorite'}</Focusable>
